...
function tileLayerCordova(L) {
    L.tileLayerCordova = function(url, options, success_callback) {
        if (!window.requestFileSystem) {
            return new L.TileLayer.CordovaFallback(url, options, success_callback);
        }
        return new L.TileLayer.Cordova(url, options, success_callback);
    }

    L.TileLayer.CordovaFallback = L.TileLayer.extend({
    ...
    L.TileLayer.Cordova = L.TileLayer.extend({
        initialize: function(url, options, success_callback) {
            ...
            var myself = this;
            myself.header = this.options.header ? this.options.header : null;
            ...
            window.requestFileSystem(
                ...
                function(fshandle) {
                    ...
                    myself.fshandle.root.getDirectory(
                        options.folder, { create: true, exclusive: false },
                        function(dirhandle) {
                            ...
                            //myself._url_offline = dirhandle.toInternalURL() + '/' + [myself.options.name, '{z}', '{x}', '{y}'].join('-') + '.png';
                            myself._url_offline = dirhandle.toInternalURL() + [myself.options.name, '{z}', '{x}', '{y}'].join('-') + '.png';
        ...
        downloadAndStoreTile: function(x, y, z, success_callback, error_callback) {
            var myself = this;
            var filename = myself.dirhandle.toInternalURL() + [myself.options.name, z, x, y].join('-') + '.png';
